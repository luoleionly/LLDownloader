...
```ruby
pod 'LLDownloader', :path => '../LLDownloader'
```

Or from a git repo:

```ruby
pod 'LLDownloader', :git => 'https://github.com/<you>/LLDownloader.git'
```

### Manual

Drag the `LLDownloader/` folder into your Xcode project (choose *Create groups*, link to your app target). All files are ARC-enabled. Then:
...
| `LLDownloadTask` | Single download. Exposes `status` / `progress` / `speed` / `fileName` / `filePath` / `response`, chainable callbacks, and KVO-observable `NSProgress`. |
...
Errors live in the `LLErrorDomain` domain with codes in `LLErrorCode` (see `LLError.h`). Additional structured keys:
